...
class pyomo_model:
    ...
    def define_model(self):
        ...
        self.model.p = pyo.Param(
            self.model.I, initialize=self.param_pop
        ...
        self.model.a = pyo.Var(
            self.model.I,
            self.model.J,
            domain=pyo.Binary,
            initialize=self.param_adjacent,
        )  # 1 if county i and j are adjacent

        self.model.x = pyo.Var(
            self.model.J, domain=pyo.Binary
        )  # 1 if principal place of business is opened in county j
        self.model.y = pyo.Var(
            self.model.I, domain=pyo.Binary
        )  # 1 if county i is covered
